...
/obj/effect/track
	...
	icon = 'icons/effects/track.dmi' //This sucks, but too bad!
...
/obj/effect/track/attack_hand(mob/living/user)
	...
	to_chat(user, "You start concealing the tracks..")
	if(!do_after(user, 4 SECONDS, target = src))
		return
	to_chat(user, "Nobody should be able to follow these tracks anymore..")
	qdel(src)
	return TRUE
...
/obj/effect/track/examine(mob/user)
	...
	if(knowledge > ANALYSIS_TERRIBLE && creator == user)
		. += "These are your own tracks!"
	if(knowledge >= ANALYSIS_GOOD)
		if(overwrites > 10)
			. += "There are traces of many older tracks here, too.."
		else if(overwrites >= 2)
			. += "There are traces of around [overwrites] older tracks here, too.."
